...
@dp.message(lambda message: message.text == '9.1t')
async def send_presentation_for_first_theme(message: types.Message):
    ...
    document = FSInputFile(path='utils\\media\\Tasks\\9class\\9.1\\tasks9_1.txt')
    await bot.send_document(message.chat.id, document = document)


@dp.message(lambda message: message.text == '9.2t')
async def send_presentation_for_second_theme(message: types.Message):
    ...
    document = FSInputFile(path='utils\\media\\Tasks\\9class\\9.2\\tasks9_2.txt')
    await bot.send_document(message.chat.id, document = document)


@dp.message(lambda message: message.text == '9.3t')
async def send_presentation_for_third_theme(message: types.Message):
    ...
    document = FSInputFile(path='utils\\media\\Tasks\\9class\\9.3\\tasks9_3.txt')
    await bot.send_document(message.chat.id, document = document)
