...
@dp.message(lambda message: message.text == '11.1p')
async def send_presentation_for_first_theme(message: types.Message):
    ...
    document = FSInputFile(path='utils\\media\\Presentations\\11class\\11.1\\pres1.txt')
    await bot.send_document(message.chat.id, document = document)


@dp.message(lambda message: message.text == '11.2p')
async def send_presentation_for_second_theme(message: types.Message):
    ...
    document = FSInputFile(path='utils\\media\\Presentations\\11class\\11.2\\pres2.txt')
    await bot.send_document(message.chat.id, document = document)
